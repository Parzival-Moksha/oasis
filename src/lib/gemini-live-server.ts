  ...
  const declarations: GeminiLiveFunctionDeclaration[] = [
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          type: { type: 'string', enum: ['catalog', 'crafted', 'portal', 'spatial-web', 'light', 'agent-avatar'], description: 'Optional object type filter.' },
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          visibility: { type: 'string', enum: ['unlisted', 'public', 'public_edit', 'private'], description: 'Share visibility. Use unlisted for hackathon links.' },
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          worldId: { type: 'string', description: 'Optional world ID. Omit to use the active browser world.' },
          visibility: { type: 'string', enum: ['unlisted', 'public', 'public_edit', 'private'], description: 'Share visibility.' },
          publicBaseUrl: { type: 'string', description: 'Optional public Oasis base URL, for example https://04515.xyz.' },
        },
        additionalProperties: false,
      },
...
export function buildGeminiLiveSystemInstruction(args: { worldId?: string; worldName?: string; model?: string; voice?: string } = {}): string {
  ...
  return [
    ...
    'Use query_objects before modify_object, remove_object, set_behavior, or modify_light unless the user gives an exact ID.',
