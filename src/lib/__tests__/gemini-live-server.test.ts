...
describe('gemini live server guardrails', () => {
  it('keeps Gemini Live model selection scoped to known Live models', async () => {
    ...
    expect(sanitizeGeminiLiveModel('gemini-2.5-flash-native-audio-preview-12-2025')).toBe('gemini-3.1-flash-live-preview')
  })
  ...
  it('declares Oasis tools, voice config, and token transport in the session manifest', async () => {
    ...
    expect(manifest.toolDeclarationNames).toContain('place_agent_window')
    expect(manifest.toolDeclarationNames).toContain('place_browser_window')
    expect(manifest.toolDeclarationNames).toContain('text_to_pic')
    expect(manifest.toolDeclarationNames).toContain('text_to_pic_building')
    expect(manifest.toolDeclarationNames).toContain('text_to_video')
    expect(manifest.toolDeclarationNames).toContain('text_to_music')
    expect(manifest.toolDeclarationNames).toContain('place_media')
    ...
    expect(JSON.stringify(manifest.setupMessage)).toContain('place_browser_window')
    expect(JSON.stringify(manifest.setupMessage)).toContain('text_to_pic_building')
  ...
  it('keeps Gemini function schemas inside the Live API subset', async () => {
    ...
    expect(serialized).toContain('place_agent_window')
    expect(serialized).toContain('place_browser_window')
    expect(serialized).toContain('text_to_pic')
    expect(serialized).toContain('text_to_pic_building')
    expect(serialized).toContain('text_to_video')
    expect(serialized).toContain('text_to_music')
    expect(serialized).toContain('place_media')
