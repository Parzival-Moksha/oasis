...
export function QuestProgressTracker({ activeWorldId }: { activeWorldId: string }) {
  ...
  const showToast = useCallback((title: string, message: string, tone: ToastState['tone'] = 'quest') => {
  ...
  useEffect(() => {
    ...
    const onToast = (event: Event) => {
      ...
      showToast(detail?.title || 'Quest updated', detail?.message || 'Progress saved', detail?.tone || 'quest')
    }
  ...
  return (
    <>
      <div className="pointer-events-none fixed right-4 top-20 z-[184] w-[min(330px,calc(100vw-2rem))] select-none rounded-lg border border-orange-200/20 bg-black/70 p-3 text-white shadow-[0_0_36px_rgba(249,115,22,0.18)] backdrop-blur-md">
      ...
      {toast && (
        <div className="pointer-events-none fixed left-1/2 top-8 z-[230] w-[min(360px,calc(100vw-2rem))] -translate-x-1/2 select-none rounded-lg border border-orange-200/35 bg-black/82 p-3 text-center text-white shadow-[0_0_42px_rgba(249,115,22,0.28)] backdrop-blur-md">
