...
const PHASES: Phase[] = [
  ...
  },
  {
    ...
    details: [
      ...
      { title: 'Cursor control in zoomon mode', description: 'Mouse should become standard 2D cursor when focused on a 3D agent window.', status: 'planned' },
      { title: 'Avatar TPS cannot interact with 3D windows', description: 'Third-person avatar needs selection/interaction with 3D windows.', status: 'planned' },
      { title: 'Window occlusion (occlude="blending")', description: 'Try occlude="blending" with correct sizing for proper depth sorting.', status: 'planned' },
      { title: '3D window flicker during streaming', description: 'Html component re-renders every frame during SSE streaming + camera movement.', status: 'planned' },
      { title: 'Extract frame components (FrameComponents.tsx)', description: 'Break circular import by extracting frame rendering to dedicated file.', status: 'planned' },
      // ── Audio ──
